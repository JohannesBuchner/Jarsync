...
dnl
dnl Determines what JNI stub generator is installed.
AC_DEFUN(AC_PROG_JAVAH,[
AC_REQUIRE([AC_EXEEXT])dnl
AC_ARG_VAR(JAVAH,      [Java native stub generator.])
test x$JAVAH = x && AC_CHECK_PROGS(JAVAH, javah$EXEEXT, gcjh$EXEEXT)
test x$JAVAH = x && AC_MSG_ERROR([no acceptable JNI stub generator found in \$PATH])
if test "$JAVAH" = "javah$EXEEXT"; then
	JAVAHCP='-classpath '
else
	JAVAHCP='--jni -I'
fi

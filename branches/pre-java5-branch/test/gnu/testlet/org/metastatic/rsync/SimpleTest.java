 ...
 * @version $Revision $
 */
public class SimpleTest implements Testlet, MatcherListener {
   ...
   public void test(TestHarness harness) {
      ...
      for (int i = 0; i < 50; i++) {
         ...
         harness.verbose("\tGenerated " + sums.size() + " checksums.");
         ...
         harness.verbose("\tDeltas: " + copies + " copy commands, "
         ...
         if (!Arrays.equals(n3w, reconst)) {
            ...
            if (n3w.length != reconst.length) {
               harness.debug("\tThe arrays are not of equal length!");
               harness.debug("\t    n3w=" + n3w.length);
               harness.debug("\treconst=" + reconst.length);
               harness.debug("\tThe two differ in size by " +
                  (n3w.length - reconst.length) + " bytes.");
            } else {
               for (int x = 0; x < n3w.length; x++) {
                  if (n3w[x] != reconst[x]) {
                     harness.debug("diff: index=" + x +
                        " n3w=" + Util.toHexString(n3w, x, 1) +
                        " reconst=" + Util.toHexString(reconst, x, 1)
                     //   + " " + y + " contiguous mismatched blocks so far."
